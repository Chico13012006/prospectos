    ...
    <div className="p-6 space-y-5">
      <div className="animate-in stagger-1">
        <h1 className="text-2xl font-bold text-slate-100 flex items-center gap-2">
          <Sparkles size={22} className="text-indigo-400" /> Copiloto pós-reunião
        </h1>
        <p className="text-sm text-slate-400 mt-0.5">
          Cole a transcrição da reunião (gerada pelo Google Meet). A IA resume, extrai dores,
          equipamentos e objeções, e sugere próximos passos — tudo revisável antes de aplicar.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Entrada */}
        <div className="card p-5 space-y-3 animate-in stagger-2">
          <div>
...
function Resultado({ analise, lead }: { analise: AnaliseReuniao; lead: Lead | null }) {
  ...
  const simuladorHref = analise.equipamentos.length > 0
    ? `/simulador?modelo=comodato&itens=${analise.equipamentos.map((e) => `${e.produto}:${e.quantidade}`).join(',')}`
    : null;
