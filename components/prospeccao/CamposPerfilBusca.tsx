...
import { AlertCircle, Check, ChevronDown, Info, Lock, MapPin, Plus, Sparkles, X } from 'lucide-react';
...
// Campos do perfil de busca (nichos, estados, municípios, porte e opções).
// Um só editor para o painel lateral da Prospecção e a aba de Configurações:
// quem chama carrega e salva o perfil; aqui só se edita o objeto.

const LIMITE = PROSPECCAO_LIMITES.cnaes;

export default function CamposPerfilBusca({
  perfil, onChange, podeEditar, catalogoCnaes = null, colunasNichos = 2,
}: {
  perfil: ProspeccaoConfig;
  onChange: (atualizar: (p: ProspeccaoConfig) => ProspeccaoConfig) => void;
  podeEditar: boolean;
  ...
  function alternarPorte(valor: PorteProspeccao) {
    onChange((p) => {
      const atual = p.portes ?? [];
      return { ...p, portes: atual.includes(valor) ? atual.filter((v) => v !== valor) : [...atual, valor] };
    });
  }
  ...
  return (
    <>
      ...
      <section className={s.drawerSection}>
        ...
        <div className={s.nichoGrid} style={colunasNichos === 3 ? { gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))' } : undefined}>
          {NICHOS.map((n) => {
      ...
      <section className={s.drawerSection}>
        ...
        <SeletorEstados
          ...
          onChange={(novas) => onChange((p) => ({ ...p, ufs: novas }))}
        />
      </section>
      ...
      <section className={s.drawerSection}>
        ...
        <SeletorMunicipios
          ...
          ufs={ufs}
          desabilitado={!podeEditar}
          onChange={(novos) => onChange((p) => ({ ...p, municipios: novos }))}
        />
      ...
      <section className={s.drawerSection}>
        ...
        <div className="flex flex-wrap gap-1.5">
          {PORTES_PROSPECCAO.map((p: PorteProspeccao) => (
            <button key={p} type="button" disabled={!podeEditar} onClick={() => alternarPorte(p)} aria-pressed={portes.includes(p)} className={chip(portes.includes(p))}>
              {ROTULO_PORTE[p]}
            </button>
          ))}
        </div>
        <div className="mt-4 grid gap-2">
          <button type="button" disabled={!podeEditar} onClick={() => onChange((p) => ({ ...p, excluirMei: !p.excluirMei }))} aria-pressed={!!perfil.excluirMei} className={`${s.optionRow} ${perfil.excluirMei ? s.optionRowAtivo : ''} focus-ring`}>
          ...
          <button type="button" disabled={!podeEditar} onClick={() => onChange((p) => ({ ...p, incluirCnaesSecundarios: !p.incluirCnaesSecundarios }))} aria-pressed={!!perfil.incluirCnaesSecundarios} className={`${s.optionRow} ${perfil.incluirCnaesSecundarios ? s.optionRowAtivo : ''} focus-ring`}>
